...
function UserLiveStreaming() {
  ...
  const fetchStreams = async () => {
    try {
      ...
      console.log('User Live Streams API Response:', result);
      if (result.status === "success" && result.data) {
        setStreams(result.data);
        localStorage.setItem('userLiveStreams', JSON.stringify(result.data));
      }
    } catch (error) {
  ...
  const refreshStreams = () => {
    setLoading(true);
    localStorage.removeItem('userLiveStreams');
    fetchStreams();
  };
